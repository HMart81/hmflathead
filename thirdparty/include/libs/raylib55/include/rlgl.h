...
// RLTexture parameters (equivalent to OpenGL defines)
...
#define RL_TEXTURE_MIPMAP_BIAS_RATIO            0x4000      // RLTexture mipmap bias, percentage ratio (custom identifier)
...
typedef struct rlDrawCall {
    ...
    //unsigned int shaderId;    // RLShader id to be used on the draw -> Using RLGL.currentShaderId
    unsigned int textureId;     // RLTexture id to be used on the draw -> Use to create new draw call if changes
...
typedef enum {
    ...
    RL_SHADER_UNIFORM_UINT,             // RLShader uniform type: unsigned int
    RL_SHADER_UNIFORM_UIVEC2,           // RLShader uniform type: uivec2 (2 unsigned int)
    RL_SHADER_UNIFORM_UIVEC3,           // RLShader uniform type: uivec3 (3 unsigned int)
    RL_SHADER_UNIFORM_UIVEC4,           // RLShader uniform type: uivec4 (4 unsigned int)
...
RLAPI void rlColorMask(bool r, bool g, bool b, bool a); // RLColor mask control
...
// RLShader buffer storage object management (ssbo)
...
typedef struct rlglData {
    ...
    struct {
        ...
        RLMatrix transform;                   // RLTransform matrix to be used with rlTranslate, rlRotate, rlScale
    ...
    struct {
        ...
        bool ssbo;                          // RLShader storage buffer object support (GL_ARB_shader_storage_buffer_object)
...
#define rlMatrixToFloat(mat) (rlMatrixToFloatV(mat).v)      // Get float vector for RLMatrix
...
void rlVertex3f(float x, float y, float z)
{
    ...
    // RLTransform provided vector if required
...
// NOTE: RLTexture coordinates are limited to QUADS only
...
{
    // RLColor values clamp to 0.0f(0) and 1.0f(255)
...
{
    glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);     // Clear used buffers: RLColor and Depth (Depth is used for 3D)
...
static void GLAPIENTRY rlDebugMessageCallback(GLenum source, GLenum type, GLuint id, GLenum severity, GLsizei length, const GLchar *message, const void *userParam)
{
    ...
    // - #131204 - RLTexture state usage warning: The texture object (0) bound to texture image unit 0 does not have
...
void rlglInit(int width, int height)
{
    ...
    // Init default RLShader (customized for GL 3.3 and ES2)
    ...
    glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);      // RLColor blending function (how colors are mixed)
...
#if defined(GRAPHICS_API_OPENGL_11)
    // Init state: RLColor hints (deprecated in OpenGL 3.0+)
    ...
    // Init state: RLColor/Depth buffers clear
...
void rlLoadExtensions(void *loader)
{
    ...
    RLGL.ExtSupported.texCompDXT = GLAD_GL_EXT_texture_compression_s3tc;  // RLTexture compression: DXT
    RLGL.ExtSupported.texCompETC2 = GLAD_GL_ARB_ES3_compatibility;        // RLTexture compression: ETC2/EAC
    #if defined(GRAPHICS_API_OPENGL_43)
    ...
    if (RLGL.ExtSupported.ssbo) TRACELOG(RL_LOG_INFO, "GL: RLShader storage buffer objects supported");
#endif  // RLGL_SHOW_GL_DETAILS_INFO
...
unsigned int rlLoadTexture(const void *data, int width, int height, int format, int mipmapCount)
{
    ...
    // RLTexture parameters configuration
    ...
    if (id > 0) TRACELOG(RL_LOG_INFO, "TEXTURE: [ID %i] RLTexture loaded successfully (%ix%i | %s | %i mipmaps)", id, width, height, rlGetPixelFormatName(format), mipmapCount);
...
void *rlReadTexturePixels(unsigned int id, int width, int height, int format)
{
    ...
    // RLTexture width and height are required on OpenGL ES 2.0, there is no way to get it from texture id
...
// NOTE: Attach type: 0-RLColor, 1-Depth renderbuffer, 2-Depth texture
...
unsigned int rlLoadShaderCode(const char *vsCode, const char *fsCode)
{
    ...
    else if ((vertexShaderId > 0) && (fragmentShaderId > 0))
    {
        ...
        {
            // WARNING: RLShader program linkage could fail and returned id is 0
...
int rlGetLocationUniform(unsigned int shaderId, const char *uniformName)
{
    ...
    //else TRACELOG(RL_LOG_INFO, "SHADER: [ID %i] RLShader uniform (%s) set at location: %i", shaderId, uniformName, location);
#endif
    return location;
}
...
int rlGetLocationAttrib(unsigned int shaderId, const char *attribName)
{
    ...
    //else TRACELOG(RL_LOG_INFO, "SHADER: [ID %i] RLShader attribute (%s) set at location: %i", shaderId, attribName, location);
#endif
    return location;
}
...
#else
    TRACELOG(RL_LOG_WARNING, "TEXTURE: RLImage texture binding not enabled. Define GRAPHICS_API_OPENGL_43");
#endif
}
